...
if (isset($_POST['tambah'])) {
    ...
    $kode_gol = trim($_POST['kode_gol'] ?? '');

    if ($nama_pangkat !== "" && $kode_gol !== "") {

        $nama_pangkat = mysqli_real_escape_string($conn, $nama_pangkat);
        $kode_gol = mysqli_real_escape_string($conn, $kode_gol);

        mysqli_query($conn,
            "INSERT INTO master_golongan
             (nama_pangkat, kode_gol)
             VALUES ('$nama_pangkat', '$kode_gol')"
        );

        header("Location: Admin_DM_Golongan.php");
        exit;
    }
}
...
if (isset($_GET['hapus'])) {
    ...
    mysqli_query($conn, "DELETE FROM master_golongan WHERE id_gol = $id");
...
if (isset($_POST['update'])) {
    ...
    $nama_pangkat = trim($_POST['nama_pangkat_edit'] ?? '');
    $kode_gol = trim($_POST['kode_gol_edit'] ?? '');

    if ($id > 0 && $nama_pangkat !== '' && $kode_gol !== '') {

        $nama_pangkat = mysqli_real_escape_string($conn, $nama_pangkat);
        $kode_gol = mysqli_real_escape_string($conn, $kode_gol);

        mysqli_query($conn,
            "UPDATE master_golongan
            SET nama_pangkat = '$nama_pangkat',
                kode_gol = '$kode_gol'
            WHERE id_gol = $id"
        );
    }
...
// Hitung total data
$totalQuery = mysqli_query($conn, "SELECT COUNT(*) as total FROM master_golongan");
...
$query = mysqli_query($conn,
    "SELECT id_gol, nama_pangkat, kode_gol
...
<aside class="sidebar" id="sidebar">
    ...
    <div class="submenu aktif" id="submenuDataMaster">
        ...
        <a href="Admin_DM_Golongan.php" class="item-submenu aktif">Golongan</a>
...
<div class="header-master">
    <span class="judul-master">Manage Golongan</span>
...
<table width="100%" class="tabel-master">
    ...
    <tr>
        ...
        <td><?= htmlspecialchars($row['kode_gol']); ?></td>

        <td>
            <button type="button"
                    class="tombol-ubah"
                    onclick="openEditModal(<?= $row['id_gol']; ?>, 
                    '<?= htmlspecialchars($row['nama_pangkat']); ?>',
                    '<?= htmlspecialchars($row['kode_gol']); ?>'
                    )">
            ...
            <button type="button"
                    class="tombol-hapus"
                    onclick="openDeleteModal(<?= $row['id_gol']; ?>)">
...
<div id="modalTambah" class="modal">
    <div class="modal-content">
        ...
        <h3 style="margin-top:0;">Tambah Golongan</h3>
...
function openEditModal(id, nama, jenis) {
    ...
    document.getElementById("editNamaPangkat").value = nama;
    document.getElementById("editKodeGolongan").value = jenis;
