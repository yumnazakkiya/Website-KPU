...
if (isset($_POST['tambah'])) {
    ...
    if ($jenis_kelamin !== "") {
        ...
        $insert = mysqli_query(
            $conn,
            "INSERT INTO master_jenis_kelamin (jenis_kelamin) 
        ...
        header("Location: Admin_DM_Gender.php");
        exit;
    }
}
...
if (isset($_GET['hapus'])) {
    ...
    mysqli_query($conn, "DELETE FROM master_jenis_kelamin WHERE id_jenis_kelamin = $id");
...
if (isset($_POST['update'])) {
    ...
    $jenis_kelamin = trim($_POST['jenis_kelamin_edit'] ?? '');

    if ($id > 0 && $jenis_kelamin !== '') {

        $jenis_kelamin = mysqli_real_escape_string($conn, $jenis_kelamin);

        mysqli_query($conn,
            "UPDATE master_jenis_kelamin 
...
// Hitung total data
$totalQuery = mysqli_query($conn, "SELECT COUNT(*) as total FROM master_jenis_kelamin");
...
$query = mysqli_query($conn,
    "SELECT id_jenis_kelamin, jenis_kelamin
     FROM master_jenis_kelamin
     ORDER BY id_jenis_kelamin
...
if (isset($_POST['id_delete'])) {
    ...
    if (!empty($ids)) {
        ...
        mysqli_query($conn,
            "DELETE FROM master_jenis_kelamin 
             WHERE id_jenis_kelamin IN ($idList)"
        );
    }
...
<aside class="sidebar" id="sidebar">
    ...
    <div class="submenu aktif" id="submenuDataMaster">
        <a href="Admin_DM_Gender.php" class="item-submenu aktif">Jenis Kelamin</a>
...
<div class="header-master">
    <span class="judul-master">Manage Jenis Kelamin</span>
...
<table width="100%" class="tabel-master">
    ...
    <tr>
        ...
        <td>
            <button type="button"
                    class="tombol-ubah"
                    onclick="openEditModal(<?= $row['id_jenis_kelamin']; ?>, '<?= htmlspecialchars($row['jenis_kelamin']); ?>')">
                <i class="fa-solid fa-pen"></i>
            </button>

            <button type="button"
                    class="tombol-hapus"
                    onclick="openDeleteModal(<?= $row['id_jenis_kelamin']; ?>)">
...
<div id="modalTambah" class="modal">
    <div class="modal-content">
        ...
        <h3 style="margin-top:0;">Tambah Jenis Kelamin</h3>
...
function openEditModal(id, nama) {
    ...
    document.getElementById("editJenisKelamin").value = nama;
