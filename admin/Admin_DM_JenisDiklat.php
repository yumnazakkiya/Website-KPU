...
if (isset($_POST['tambah'])) {
    ...
    $diklat = trim($_POST['diklat'] ?? '');

    if ($diklat !== "") {

        $diklat = mysqli_real_escape_string($conn, $diklat);

        $insert = mysqli_query(
            $conn,
            "INSERT INTO master_diklat (jenis_diklat) 
             VALUES ('$diklat')"
        );
        ...
        header("Location: Admin_DM_JenisDiklat.php");
        exit;
    }
}
...
if (isset($_GET['hapus'])) {
    ...
    mysqli_query($conn, "DELETE FROM master_diklat WHERE id_jenis_diklat = $id");
    ...
    if ($id > 0 && $diklat !== '') {
        ...
        mysqli_query($conn,
            "UPDATE master_diklat
             SET jenis_diklat = '$diklat' 
             WHERE id_jenis_diklat = $id"
        );
    }
...
$totalQuery = mysqli_query($conn, "SELECT COUNT(*) as total FROM master_diklat");
$totalData = mysqli_fetch_assoc($totalQuery)['total'];

$totalPages = ceil($totalData / $limit);

$query = mysqli_query($conn,
    "SELECT id_jenis_diklat, jenis_diklat
     FROM master_diklat
     ORDER BY id_jenis_diklat
...
if (isset($_POST['id_delete'])) {
    ...
    if (!empty($ids)) {
        ...
        mysqli_query($conn,
            "DELETE FROM master_diklat
             WHERE id_jenis_diklat IN ($idList)"
        );
    }
...
<aside class="sidebar" id="sidebar">
    ...
    <div class="submenu aktif" id="submenuDataMaster">
        ...
        <a href="Admin_DM_JenisDiklat.php" class="item-submenu aktif">Jenis Diklat</a>
...
<div class="header-master">
    <span class="judul-master">Manage Jenis Diklat</span>
...
<table width="100%" class="tabel-master">
    ...
    <tr>
        ...
        <td>
            <button type="button"
                    class="tombol-ubah"
                    onclick="openEditModal(<?= $row['id_jenis_diklat']; ?>, '<?= htmlspecialchars($row['jenis_diklat']); ?>')">
                <i class="fa-solid fa-pen"></i>
            </button>

            <button type="button"
                    class="tombol-hapus"
                    onclick="openDeleteModal(<?= $row['id_jenis_diklat']; ?>)">
...
<div id="modalTambah" class="modal">
    <div class="modal-content">
        ...
        <h3 style="margin-top:0;">Tambah Jenis Diklat</h3>
...
function openEditModal(id, nama) {
    ...
    document.getElementById("editDiklat").value = nama;
