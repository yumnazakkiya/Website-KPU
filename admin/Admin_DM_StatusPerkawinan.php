...
if (isset($_POST['tambah'])) {
    ...
    if ($status_perkawinan !== "") {
        ...
        $status_perkawinan = mysqli_real_escape_string($conn, $status_perkawinan);

        $insert = mysqli_query(
            $conn,
            "INSERT INTO master_status_perkawinan (status_perkawinan) 
        ...
        header("Location: Admin_DM_StatusPerkawinan.php");
        exit;
    }
}
...
if (isset($_GET['hapus'])) {
    ...
    mysqli_query($conn, "DELETE FROM master_status_perkawinan WHERE id_status_perkawinan = $id");
...
if (isset($_POST['update'])) {
    ...
    $status_perkawinan = trim($_POST['status_perkawinan_edit'] ?? '');

    if ($id > 0 && $status_perkawinan !== '') {

        $status_perkawinan = mysqli_real_escape_string($conn, $status_perkawinan);

        mysqli_query($conn,
            "UPDATE master_status_perkawinan
             SET status_perkawinan = '$status_perkawinan' 
             WHERE id_status_perkawinan = $id"
        );
    }
...
// Hitung total data
$totalQuery = mysqli_query($conn, "SELECT COUNT(*) as total FROM master_status_perkawinan");
...
$query = mysqli_query($conn,
    "SELECT id_status_perkawinan, status_perkawinan
     FROM master_status_perkawinan
     ORDER BY id_status_perkawinan
...
if (isset($_POST['id_delete'])) {
    ...
    if (!empty($ids)) {
        ...
        mysqli_query($conn,
            "DELETE FROM master_status_perkawinan 
             WHERE id_status_perkawinan IN ($idList)"
        );
    }
...
<head>
    <meta charset="UTF-8">
    <title>Manage Status Perkawinan</title>
    <link rel="stylesheet" href="../assets/data_master.css">
...
<aside class="sidebar" id="sidebar">
    ...
    <div class="submenu aktif" id="submenuDataMaster">
        ...
        <a href="Admin_DM_StatusPerkawinan.php" class="item-submenu aktif">Status Perkawinan</a>
...
<div class="header-master">
    <span class="judul-master">Manage Status Perkawinan</span>
...
<table width="100%" class="tabel-master">
    ...
    <tr>
        <td>
            <input type="checkbox" 
                   name="id_delete[]" 
                   value="<?= $row['id_status_perkawinan']; ?>">
        </td>
        ...
        <td>
            <button type="button"
                    class="tombol-ubah"
                    onclick="openEditModal(<?= $row['id_status_perkawinan']; ?>, '<?= htmlspecialchars($row['status_perkawinan']); ?>')">
                <i class="fa-solid fa-pen"></i>
            </button>

            <button type="button"
                    class="tombol-hapus"
                    onclick="openDeleteModal(<?= $row['id_status_perkawinan']; ?>)">
...
<div id="modalTambah" class="modal">
    <div class="modal-content">
        ...
        <h3 style="margin-top:0;">Tambah Status Perkawinan</h3>
...
<div id="modalEdit" class="modal">
    <div class="modal-content">
        ...
        <h3>Edit Status Perkawinan</h3>

        <form method="POST">
            <input type="hidden" name="id_edit" id="editId">

            <input type="text"
                   name="status_perkawinan_edit"
                   id="editStatusPerkawinan"
                   required
                   style="width:100%; height:35px; margin-bottom:15px; padding:5px;">
...
function openEditModal(id, nama) {
    ...
    document.getElementById("editStatusPerkawinan").value = nama;
