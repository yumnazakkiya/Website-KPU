...
if (isset($_POST['tambah'])) {
    ...
    if ($predikat !== "") {
        ...
        $predikat = mysqli_real_escape_string($conn, $predikat);

        $insert = mysqli_query(
            $conn,
            "INSERT INTO master_predikat_skp (predikat_skp)
             VALUES ('$predikat')"
        );
        ...
        header("Location: Admin_DM_PredikatSKP.php");
        exit;
    }
}
...
if (isset($_GET['hapus'])) {
    ...
    mysqli_query($conn, "DELETE FROM master_predikat_skp WHERE id_predikat_skp = $id");
...
if (isset($_POST['update'])) {
    ...
    $predikat = trim($_POST['predikat_edit'] ?? '');
    ...
    if ($id > 0 && $predikat !== '') {
        ...
        mysqli_query($conn,
            "UPDATE master_predikat_skp
             SET predikat_skp = '$predikat' 
             WHERE id_predikat_skp = $id"
        );
    }
...
// Hitung total data
$totalQuery = mysqli_query($conn, "SELECT COUNT(*) as total FROM master_predikat_skp");
...
$query = mysqli_query($conn,
    "SELECT id_predikat_skp, predikat_skp
     FROM master_predikat_skp
     ORDER BY id_predikat_skp
...
if (isset($_POST['id_delete'])) {
    ...
    if (!empty($ids)) {
        ...
        mysqli_query($conn,
            "DELETE FROM master_predikat_skp 
             WHERE id_predikat_skp IN ($idList)"
        );
    }
...
<head>
    <meta charset="UTF-8">
    <title>Manage Predikat SKP</title>
    <link rel="stylesheet" href="../assets/data_master.css">
    <link rel="stylesheet" href="../assets/style.css" />
    <link rel="stylesheet" href="https://cdnjs.cloudflare.com/ajax/libs/font-awesome/6.5.0/css/all.min.css">
</head>
<body class="role-admin" data-jenis="predikat_skp">
...
<aside class="sidebar" id="sidebar">
    ...
    <div class="submenu aktif" id="submenuDataMaster">
        ...
        <a href="Admin_DM_PredikatSKP.php" class="item-submenu aktif">Predikat SKP</a>
...
<div class="header-master">
    <span class="judul-master">Manage Predikat SKP</span>
...
<table width="100%" class="tabel-master">
    ...
    <tr>
        <td>
            <input type="checkbox" 
                   name="id_delete[]" 
                   value="<?= $row['id_predikat_skp']; ?>">
        </td>

        <td><?= htmlspecialchars($row['predikat_skp']); ?></td>

        <td>
            <button type="button"
                    class="tombol-ubah"
                    onclick="openEditModal(<?= $row['id_predikat_skp']; ?>, '<?= htmlspecialchars($row['predikat_skp']); ?>')">
                <i class="fa-solid fa-pen"></i>
            </button>

            <button type="button"
                    class="tombol-hapus"
                    onclick="openDeleteModal(<?= $row['id_predikat_skp']; ?>)">
...
<div id="modalTambah" class="modal">
    <div class="modal-content">
        ...
        <h3 style="margin-top:0;">Tambah Predikat SKP</h3>

        <form method="POST" action="">
            <input type="text" 
                   name="predikat" 
                   placeholder="Masukkan Predikat SKP" 
                   required
...
function openEditModal(id, nama) {
    ...
    document.getElementById("editPredikat").value = nama;
