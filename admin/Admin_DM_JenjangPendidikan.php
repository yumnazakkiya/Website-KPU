...
if (isset($_POST['tambah'])) {
    ...
    if ($jenjang !== "") {
        ...
        $insert = mysqli_query(
            $conn,
            "INSERT INTO master_jenjang_pend (jenjang_pend) 
             VALUES ('$jenjang')"
        );
        ...
        header("Location: Admin_DM_JenjangPendidikan.php");
        exit;
    }
}
...
if (isset($_GET['hapus'])) {
    ...
    mysqli_query($conn, "DELETE FROM master_jenjang_pend WHERE id_jenjang_pend = $id");
...
if (isset($_POST['update'])) {
    ...
    $jenjang = trim($_POST['jenjang_edit'] ?? '');
    ...
    if ($id > 0 && $jenjang !== '') {
        ...
        mysqli_query($conn,
            "UPDATE master_jenjang_pend
             SET jenjang_pend = '$jenjang' 
             WHERE id_jenjang_pend = $id"
        );
    }
...
// Hitung total data
$totalQuery = mysqli_query($conn, "SELECT COUNT(*) as total FROM master_jenjang_pend");
...
$query = mysqli_query($conn,
    "SELECT id_jenjang_pend, jenjang_pend
     FROM master_jenjang_pend
     ORDER BY id_jenjang_pend
...
if (isset($_POST['id_delete'])) {
    ...
    if (!empty($ids)) {
        ...
        mysqli_query($conn,
            "DELETE FROM master_jenjang_pend 
             WHERE id_jenjang_pend IN ($idList)"
        );
    }
...
<head>
    <meta charset="UTF-8">
    <title>Manage Jenjang Pendidikan</title>
    <link rel="stylesheet" href="../assets/data_master.css">
    <link rel="stylesheet" href="../assets/style.css" />
    <link rel="stylesheet" href="https://cdnjs.cloudflare.com/ajax/libs/font-awesome/6.5.0/css/all.min.css">
</head>
<body class="role-admin" data-jenis="jenjang">
...
<aside class="sidebar" id="sidebar">
    ...
    <div class="submenu aktif" id="submenuDataMaster">
        ...
        <a href="Admin_DM_JenjangPendidikan.php" class="item-submenu aktif">Jenjang Pendidikan</a>
...
<div class="header-master">
    <span class="judul-master">Manage Jenjang Pendidikan</span>
...
<table width="100%" class="tabel-master">
    ...
    <tr>
        <td>
            <input type="checkbox" 
                   name="id_delete[]" 
                   value="<?= $row['id_jenjang_pend']; ?>">
        </td>

        <td><?= htmlspecialchars($row['jenjang_pend']); ?></td>

        <td>
            <button type="button"
                    class="tombol-ubah"
                    onclick="openEditModal(<?= $row['id_jenjang_pend']; ?>, '<?= htmlspecialchars($row['jenjang_pend']); ?>')">
                <i class="fa-solid fa-pen"></i>
            </button>

            <button type="button"
                    class="tombol-hapus"
                    onclick="openDeleteModal(<?= $row['id_jenjang_pend']; ?>)">
...
<div id="modalTambah" class="modal">
    <div class="modal-content">
        ...
        <h3 style="margin-top:0;">Tambah Jenjang Pendidikan</h3>
...
function openEditModal(id, nama) {
    ...
    document.getElementById("editJenjang").value = nama;
